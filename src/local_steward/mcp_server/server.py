"""Official low-level MCP STDIO server assembly and process entry point."""
...
def governed_config_path(environment: Mapping[str, str] | None = None) -> Path:
    """Resolve the sole installation-owned path without defaults or searching."""
...
def create_server(dispatcher: McpDispatcher) -> Server[None]:
    """Create the frozen three-tool server with empty non-tool inventories."""
    # The SDK otherwise logs untrusted unknown tool names before adapter handling.
...
async def run_stdio(config_path: Path) -> None:
    """Serve only MCP frames on standard output."""
    server = create_server(McpDispatcher(config_path))
...
def main() -> None:
    """Fail closed before MCP startup when installation authority is invalid."""
    try:
        config_path = governed_config_path()
    except Exception:
        print("STEWARD MCP configuration is unavailable.", file=sys.stderr)
